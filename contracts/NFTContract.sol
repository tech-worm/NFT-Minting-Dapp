...
contract NFTContract is ERC1155, Ownable, ERC1155Burnable {
    ...
    constructor() ERC1155("https://ipfs.io/ipfs/QmceqiJhxNmcdjg2vdikWgMREyLPjHsVyhDmmDUB5npio8") {
        //setPaymentToken("");
    }
    ...
    function CustomURI(uint256 _id) public view returns(string memory) {
        ...
        return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, Strings.toString(_id), ".json")) : "";
    }

    function mintHammer(uint256 _amount) external {
        require(paymentToken.balanceOf(msg.sender)>=tokenPrice*_amount, "Not enough balance");
